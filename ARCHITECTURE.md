...
1. Artifact 应挂在 Alert 下，因此应区分“创建 Artifact”和“将现有 Artifact 挂载到 Alert”两个动作。
2. Enrichment 可以挂到 Case、Alert、Artifact，因此领域层应分别支持 attach_enrichment，而 MCP 可以采用 `create_enrichment + attach_enrichment_to_target` 的两步模式按 target_type 路由。
3. Ticket 的逻辑重点是同步外部工单信息，不应该被设计成平台内部的完整工单系统；因此也应区分“创建 Ticket”和“将现有 Ticket 挂载到 Case”。
